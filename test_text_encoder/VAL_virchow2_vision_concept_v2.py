...
def train_one_fold(model, dl_tr, dl_val, dl_te, mode: str):
    ...
    for ep in range(EPOCHS):
        ...
        with torch.no_grad():
            for batch in dl_val:
                ...
                y_true_val.append(int(y.item()))
                y_prob_val.append(prob)
...
for mode in EXPERIMENTS:
    ...
    for fold_dir in fold_dirs:
        ...
        # fit_slides = train_slides + val_slides # og
        # use val separately for model selection
                # -------- datasets per mode --------
        if mode in ["vision_pool", "vision_abmil"]:
            ds_tr  = VisionOnlyBagDataset(train_slides, bag_cap=BAG_CAP, topk=TOPK, seed=SEED)
        ...
        m = train_one_fold(model, dl_tr, dl_val, dl_te, mode)
