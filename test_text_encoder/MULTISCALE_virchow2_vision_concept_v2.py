...
# PT_FEAT_DIR = r"C:\Users\Vivian\Documents\CONCH\conch_img_feats\10x_unnorm_feats\20x\pt"        # CONCH 10x unnorm
...
PT_FEAT_DIRS = {
    "2.5x": r'C:\Users\Vivian\Documents\CONCH\conch_img_feats\25x_unnorm_feats\20x\pt',
    # "5x": r'C:\Users\Vivian\Documents\CONCH\conch_img_feats\5x_unnorm_feats\20x\pt',
    "10x": r"C:\Users\Vivian\Documents\CONCH\conch_img_feats\10x_unnorm_feats\20x\pt"  
}

PATCH_CONCEPT_CSVS = {
    "2.5x": r'C:\Users\Vivian\Documents\CONCH\test_text_encoder\slide_concept_scores\2.5x\noCAP_patch_ptfile_notopk_conf_PATCH.csv',
    # "5x": r'C:\Users\Vivian\Documents\CONCH\test_text_encoder\slide_concept_scores\5x\noCAP_patch_ptfile_notopk_conf_PATCH.csv',
    "10x": r"C:\Users\Vivian\Documents\CONCH\test_text_encoder\slide_concept_scores\noCAP_patch_ptfile_notopk_conf_PATCH.csv"
}
# ============================================
VISION_BASELINE_SCALE = "10x"
...
def load_and_preprocess_patch_csv(csv_path):
    ...
    # concept cols
    concept_cols = sorted(
        [c for c in df.columns if c.startswith(CONCEPT_PREFIX)],
    ...
    # pooling (UNCHANGED logic)
    pooled_concepts = max_pool_variants(df, concept_cols, variants_per_concept=5)

    # keep metadata
    meta_cols = [SLIDE_COL, LABEL_COL]
    if X_COL in df.columns: meta_cols.append(X_COL)
    if Y_COL in df.columns: meta_cols.append(Y_COL)

    df = pd.concat([df[meta_cols].copy(), pooled_concepts], axis=1)

    return df

df_patch_dict = {}
for scale in SCALES:
    df_patch_dict[scale] = load_and_preprocess_patch_csv(
        PATCH_CONCEPT_CSVS[scale]
    )
# for scale in SCALES:
#     df = pd.read_csv(PATCH_CONCEPT_CSVS[scale])
#     df.columns = [c.strip() for c in df.columns]
#     df_patch_dict[scale] = df

concept_cols = sorted(
    [c for c in df_patch_dict[SCALES[0]].columns if c.startswith(CONCEPT_PREFIX)],
    key=lambda s: int(s.split("_")[1])
)
...
def load_slide_deep(slide_id: str, scale: str):
    PT_FEAT_DIR = PT_FEAT_DIRS[scale]
    if FEAT_BACKEND == "pt":
...
def load_slide_deep_with_coords(slide_id, scale):
    PT_FEAT_DIR = PT_FEAT_DIRS[scale]
    if FEAT_BACKEND == "pt":
...
class VisionOnlyBagDataset(Dataset):
    def __init__(self, slide_ids, bag_cap=None, topk=None, seed=0):
        ...
        for sid in slide_ids:
            try:
                ...
                _ = load_slide_deep(sid, VISION_BASELINE_SCALE)
                keep.append(sid)
            except Exception:
                pass
        self.slide_ids = keep
    ...
    def __getitem__(self, idx):
        ...
        X = load_slide_deep(sid, VISION_BASELINE_SCALE)
        N = X.shape[0]
...
class MultiScaleVisionDataset(Dataset):
    def __init__(self, slide_ids, scales, bag_cap=None, topk=None, seed=0):
        self.scales = scales
        self.bag_cap = bag_cap
        self.topk = topk
        self.rng = np.random.default_rng(seed)
    ...
    def __getitem__(self, idx):
        ...
        for scale in self.scales:
            Xd = load_slide_deep(sid, scale)  # (N,D)
            ...
            Xd_dict[scale] = torch.from_numpy(Xd)

        y = label_from_slide_id(sid)
        return Xd_dict, torch.tensor(y, dtype=torch.long), sid
    
class MultiScaleConceptDataset(Dataset):
    def __init__(self, df_patch_dict, slide_ids, concept_cols, scales,
                 bag_cap=None, topk=None, seed=0):

        self.df_patch_dict = df_patch_dict
        self.scales = scales
    ...
    def __getitem__(self, idx):
        ...
        for scale in self.scales:
            df_patch = self.df_patch_dict[scale]

            # --- deep ---
            Xd, coords = load_slide_deep_with_coords(sid, scale)

            # --- concept ---
            d = df_patch[df_patch[SLIDE_COL] == sid]

            keys = list(zip(d[X_COL].astype(int), d[Y_COL].astype(int)))
            C = d[self.concept_cols].to_numpy(dtype=np.float32)
            concept_map = {k: C[i] for i, k in enumerate(keys)}
            ...
            Xd_dict[scale] = torch.from_numpy(Xd)
            Xc_dict[scale] = torch.from_numpy(Xc)

        y = label_from_slide_id(sid)

        return Xd_dict, Xc_dict, torch.tensor(y), sid
...
class MultiScaleLateFusionMIL(nn.Module):
    def __init__(self, deep_dim, concept_dim, scales, hid=128, drop=0.1):
        super().__init__()
        self.scales = scales

        # shared branch
        self.branch = GatedAttentionMIL(deep_dim, concept_dim, hid, drop)

        # fusion classifier (concat)
        self.cls = nn.Linear(hid * len(scales), 1)

    def forward(self, Xd_dict, Xc_dict):
        z_list = []
        ...
        for scale in self.scales:
            ...
            Hd = self.branch.phi_d(Xd)
            Hc = self.branch.phi_c(Xc)

            a = self.branch.attn_d(Hd).squeeze(1) + self.branch.attn_c(Hc).squeeze(1)
            w = torch.softmax(a, dim=0)
        ...
        logit = self.cls(z_fused).squeeze(0)
        return logit
class MultiScaleVisionABMIL(nn.Module):
    def __init__(self, in_dim, scales, hid=128, drop=0.1):
        super().__init__()
        self.scales = scales

        # shared branch across scales
        ...
        # concat pooled embeddings from all scales
        self.cls = nn.Linear(hid * len(scales), 1)

    def forward(self, Xd_dict):
        z_list = []
...
def train_one_fold(model, dl_tr, dl_te, mode: str):
    ...
    for ep in range(EPOCHS):
        for batch in dl_tr:
            ...
            if mode == "vision_pool":
            ...
            elif mode == "vision_abmil_multiscale":
                Xd_dict, y, sid = item
                Xd_dict = {k: v.to(DEVICE) for k, v in Xd_dict.items()}
                y = y.float().to(DEVICE)
                logit = model(Xd_dict)
                ...
                # MS
                Xd_dict, Xc_dict, y, sid = item

                Xd_dict = {k: v.to(DEVICE) for k,v in Xd_dict.items()}
                Xc_dict = {k: v.to(DEVICE) for k,v in Xc_dict.items()}
                y = y.float().to(DEVICE)

                logit = model(Xd_dict, Xc_dict)
...
for mode in EXPERIMENTS:
    ...
    for fold_dir in fold_dirs:
        ...
        #                                     bag_cap=BAG_CAP, topk=TOPK, seed=SEED)
            ...
            ds_tr = MultiScaleVisionDataset(
                fit_slides, SCALES,
                bag_cap=BAG_CAP, topk=TOPK, seed=SEED
            )
            ds_te = MultiScaleVisionDataset(
                test_slides, SCALES,
                bag_cap=BAG_CAP, topk=TOPK, seed=SEED
            )

        elif mode == "vision_concept_gated":
            ds_tr = MultiScaleConceptDataset(
                df_patch_dict, fit_slides, concept_cols, SCALES,
                bag_cap=BAG_CAP, topk=TOPK, seed=SEED
            )
            ds_te = MultiScaleConceptDataset(
                df_patch_dict, test_slides, concept_cols, SCALES,
                bag_cap=BAG_CAP, topk=TOPK, seed=SEED
            )    
        else:
        ...
        if mode == "vision_pool":
            ...
            Xd0, _, _ = ds_tr[0]
            scale0 = list(Xd0.keys())[0]

            model = MultiScaleVisionABMIL(
                in_dim=Xd0[scale0].shape[1],
                scales=SCALES,
                hid=HID,
                drop=DROPOUT
            ).to(DEVICE)
            ...
            model = MultiScaleLateFusionMIL(
                deep_dim=Xd0[scale0].shape[1],
                concept_dim=Xc0[scale0].shape[1],
                scales=SCALES,
                hid=HID,
                drop=DROPOUT
            ).to(DEVICE)
