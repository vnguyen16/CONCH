...
def adjust_annotation_offset(image, coords):
    ...
    # manual offset
    offset_x = abs(-89145.87) / (0.3433209)
    offset_y = abs(-66619.44) / (0.3433189)
...
def main():
    ...
    # series = 8
    # scale_factor = 4.0  # Adjust based on the series level (e.g., 4 for series 8)
    ...
    # Loading image in tiles (if needed)
    scale_factor = 1  # adjust based on downsample level
    series = 6  # select correct resolution level
    tile_size = 100
    print(f"Loading VSI image from series {series}...")
